...
- Web/PWA/API architecture, Cloud Run/Firebase direction, environments, cost guardrails, low-connectivity and recovery targets.
...
- Founder-approved whole-MVP low-fidelity responsive web/PWA design proposal, including visual tokens, role navigation, states, safety, accessibility and localisation treatment. It authorises only bounded future UI implementation after the remaining readiness gates.
- Draft public-support, public-notice, pilot agreement, pharmacy onboarding/training and verification policies.

### Still required before Claude writes the next synthetic-data-only web code
...
| Cloud foundation implementation specification | Documentation agent + product owner | **Founder-approved for synthetic implementation preparation:** [Task 3 protected-platform foundation specification](task-3-protected-platform-foundation-specification.md) and [Claude implementation brief](task-3-protected-platform-foundation-task-brief.md) cover the required trust chain, IAM, JWT/App Check, rate limits, search/maintenance, ClamAV, OpenTofu and OIDC/WIF boundaries. Final project/resource names and founder-owned environment inputs remain implementation gates; no production resource is created. |
...
| Repository readiness | Founder | **Synthetic-phase baseline recorded:** [repository readiness record](repository-readiness-record.md) verifies the private-repository workflow, single owner, issue controls, Dependabot and vulnerability-alert state. Gitleaks/Trivy CI must pass in the first implementation PR; GitHub secret scanning is unavailable and remains compensated by those checks. Before cloud-connected/sensitive code, upgrade the private repo for enforced `main` protection or separately approve public-source visibility; [the public-source review](public-source-visibility-review.md) is drafted, and required checks/default-branch/least-privilege Actions still need founder confirmation. |
| Claude readiness | Founder + Claude Code | Install Superpowers from the official marketplace, disable its optional telemetry and verify Claude reads `CLAUDE.md`/handoff before the next web task. |
| First approved Claude task brief | Documentation agent + product owner | **Approved:** [GitHub issue #6](https://github.com/SohailMoinAli/MediFind/issues/6) is the narrow, synthetic-data-only foundation task with scope, non-goals, interfaces, tests and no production/cloud deployment authority. Work begins only after the required local tool and CI readiness are verified. |
