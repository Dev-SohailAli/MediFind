...
| Cloud foundation implementation specification | Documentation agent + product owner | **Drafted for founder/documentation approval:** [Task 3 protected-platform foundation specification](task-3-protected-platform-foundation-specification.md) and [Claude implementation brief](task-3-protected-platform-foundation-task-brief.md) cover the required trust chain, IAM, JWT/App Check, rate limits, search/maintenance, ClamAV, OpenTofu and OIDC/WIF boundaries. Final project/resource names, founder inputs and repository-control path remain approval gates; no production resource is created. |
