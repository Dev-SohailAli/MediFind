...
| Test foundation specification | Documentation agent + product owner | **Task-1 specification drafted:** [first synthetic-foundation test specification](first-synthetic-foundation-test-specification.md) converts the founder-approved tool/testing decisions into exact commands, CI, evidence and zero-data rules. Founder acceptance of the document is still required before it supports a coding brief. |
