...
| --- | --- | --- | --- |
| Web application/PWA | TypeScript responsive web application with installable browser shell | Open-source/browser toolchain; no framework licence fee | Keep the API/contracts stable; add a native shell only through a separate decision |
| Native shell/store packaging | Deferred React Native/Expo option | Requires paid platform accounts and separate store-review authority | Reconsider only after a funded distribution decision; no web rewrite should depend on it |
...
| App/browser attestation | Firebase App Check with supported web/browser providers | App Check itself is no-cost, subject to attestation-provider quotas | Request quota increases or add a native attestation adapter only through a future shell decision; keep backend verification contract |
| Web notifications | Standards-based Web Push, generic payloads only | Browser/provider quotas and delivery constraints must be verified | Add native FCM/APNs only with a future native-shell decision; web API state remains authoritative |
...
| Product analytics | Server-produced privacy-minimised aggregate counters plus web operational statistics | Uses existing database/monitoring allowances; no analytics SDK or data broker | Export approved aggregates to a warehouse when volume/analysis justifies it |
...
| Pharmacy notifications | Generic Web Push plus authenticated web inbox/status | No transactional-email provider in the MVP notification path | Add native push or domain-authenticated email only after separate need, processor and cost approval |

Official references: [Firebase pricing](https://firebase.google.com/pricing), [Identity Platform pricing](https://cloud.google.com/identity-platform/pricing), [Firestore pricing](https://cloud.google.com/firestore/pricing), [Cloud Run pricing](https://cloud.google.com/run/pricing), [API Gateway pricing](https://cloud.google.com/api-gateway/pricing), [Cloud Storage pricing](https://cloud.google.com/storage/pricing), [Cloud Scheduler pricing](https://cloud.google.com/scheduler/pricing), [Cloud Tasks pricing](https://cloud.google.com/tasks/pricing), [Cloud Logging pricing](https://cloud.google.com/logging), [Secret Manager pricing](https://cloud.google.com/secret-manager/pricing), [Artifact Registry pricing](https://cloud.google.com/artifact-registry/pricing), [GitHub included usage](https://docs.github.com/en/billing/reference/product-usage-included), [Cloudflare Pages limits](https://developers.cloudflare.com/pages/platform/limits/) and [Cloudflare static asset pricing](https://developers.cloudflare.com/pages/functions/pricing/).
...
`browser/PWA -> API Gateway -> private Cloud Run API -> private Firestore/Cloud Storage/internal jobs`
...
- Do not embed an API key as a security credential in the web app/PWA. Gateway-wide/provider quotas are cost containment, not a substitute for application rate limits.
...
- The gateway's generated HTTPS hostname is used initially. A custom API domain is not required for the PWA pilot.
...
The public repository stays on GitHub Free during documentation and synthetic-only work. GitHub Actions uses Ubuntu jobs, concurrency cancellation, dependency caching and short artifact retention to remain inside the current allowance. macOS CI and signed native beta builds are not part of the current PWA baseline.
...
- applicable browser/PWA security tooling; MobSF/OWASP MASTG is added only if the future native shell is activated; and
...
| Apple/Google developer accounts | Not required for the current PWA pilot; future official store distribution is separately funded and approved. |
...
- App Check/browser-attestation use and failures, with the selected web provider's current quota evidence;
- CI minutes/artifact storage and security-scan duration; and
