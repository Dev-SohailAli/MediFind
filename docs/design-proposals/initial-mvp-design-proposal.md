...
MediFind is used in a real pharmacy-search moment, often on a variable mobile connection, by a buyer who wants a fast, honest answer and a pharmacy worker who needs to act quickly between counter interactions. The design therefore favours:
...
| **Permission request (in-app explainer)** | A sheet, shown only at the moment the buyer chooses the feature, with a plain-language reason, an "Allow" primary button (opens the OS prompt) and a "Not now" secondary; never shown at first launch |
...
- **VoiceOver/TalkBack order and focus:** Reading order follows the hierarchy documented per screen in §5 (top→bottom as written). On navigation, sheet open, validation error and status change, focus moves to the new title/message, announced via a live region for asynchronous status changes (e.g., reservation approved while Requests is open). Status chips expose an accessible name combining icon meaning + label (e.g., "Status: Low stock"), never relying on the icon glyph alone reaching the accessibility tree.
