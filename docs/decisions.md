...
| ADR-274 | 2026-08-17 | Organize implementation work into larger, coherent vertical-slice tasks and one PR per capability outcome. A slice may include related client, Worker, contracts, tests, documentation and CI changes; split only for an independent approval/security/deployment gate, materially different rollback, or genuinely unrelated release. Size is governed by coherence and reviewability, not an arbitrary file or line count. | Accepted |
