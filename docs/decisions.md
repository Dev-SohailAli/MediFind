...
| ADR-275 | 2026-08-17 | **Proposed:** define the next synthetic D1 slice as six server-only search tables (`medicine_concepts`, `pharmacy_organisations`, `pharmacy_branches`, `medicine_listings`, `public_search_projection`, `public_search_terms`) with deterministic fixtures, field-level privacy classifications, Worker-owned authorization, logical JSONL export/checksums and additive migrations. Acceptance would authorize only a read-only synthetic `GET /v1/search` and `GET /v1/listings/{id}` implementation brief; it would not authorize a Cloudflare binding, account/database provisioning, real data, authentication provider, protected records, mutation, paid usage or production release. | Proposed |
