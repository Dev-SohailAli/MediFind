...
ADR-272 is the current platform authority. It supersedes every earlier
mobile/native, Firebase/GCP, Cloud Run, API Gateway, Firestore, native-push or
store-distribution decision wherever those decisions conflict. Older rows are
retained as an auditable history and must not be used to start new work.
...
| ADR-008 | 2026-08-15 | Use a shared-codebase mobile approach for iOS and Android; select concrete vendors after privacy/compliance review. | Superseded by ADR-272 |
...
| ADR-013 | 2026-08-15 | V1 uses a shared iOS/Android mobile app for buyer, pharmacy and MediFind-admin workflows; no web dashboard is in scope. | Superseded by ADR-272 |
...
| ADR-052 | 2026-08-15 | Build a shared TypeScript React Native/Expo application for iOS and Android with a TypeScript backend/API. | Superseded by ADR-272 |
...
| ADR-055 | 2026-08-15 | Buyers use passwordless phone-code sign-in, verified-email recovery and optional device-biometric session unlock. | Superseded by ADR-272 |
| ADR-056 | 2026-08-15 | Begin with invite-only TestFlight and Google Play closed-testing distribution for the pilot. | Superseded by ADR-272 |
| ADR-057 | 2026-08-15 | Target iOS 15+ and Android 10+ for the initial mobile application. | Superseded by ADR-272 |
...
| ADR-093 | 2026-08-15 | A compromised/rooted/jailbroken device is blocked from sensitive and privileged functions while non-sensitive search may remain available where safe. | Superseded by ADR-272 |
| ADR-094 | 2026-08-15 | Critical app-security fixes can require an update before sensitive features are accessible. | Accepted |
| ADR-095 | 2026-08-15 | An independent OWASP-informed mobile/API security assessment is required before real prescription uploads; high-severity findings block launch. | Superseded by ADR-272 |
| ADR-096 | 2026-08-15 | Every user can view and revoke active devices/sessions. | Accepted |
| ADR-097 | 2026-08-15 | Privileged roles are limited to two active devices; new-device enrollment requires MFA and alerts the user. | Superseded by ADR-272 |
...
| ADR-099 | 2026-08-15 | All public traffic uses managed DDoS protection and WAF/API gateway; data stores and internal services are never publicly reachable. | Superseded by ADR-272 |
...
| ADR-103 | 2026-08-15 | Use Firebase/Google Cloud Sydney where supported for the low-cost MVP; a TypeScript backend/API enforces policy rather than broad client data access. | Superseded by ADR-272 |
...
| ADR-106 | 2026-08-15 | Use separate Firebase/GCP projects for development, beta and real-prescription production from day one. | Superseded by ADR-272 |
...
| ADR-113 | 2026-08-15 | Pilot builds are distributed only through TestFlight and Google Play closed testing, never unofficial links or files. | Superseded by ADR-272 |
...
| ADR-132 | 2026-08-15 | Maintain Firebase/GCP configuration as reviewed version-controlled infrastructure/configuration; reconcile any emergency console change immediately. | Superseded by ADR-272 |
...
| ADR-142 | 2026-08-15 | Implement privileged Firebase TOTP MFA through React Native Firebase native modules in Expo development/production builds; Expo Go is not a supported MediFind runtime. | Superseded by ADR-272 |
...
| ADR-149 | 2026-08-15 | Operate a minimal static public site for legal notices, support, status and security reporting only; core workflows remain mobile-only. | Superseded by ADR-272 |
...
| ADR-153 | 2026-08-15 | Use Firebase Crashlytics/Google Cloud error monitoring only with processor approval and strict redaction of identifiers, health data, searches, tokens and free text. | Superseded by ADR-272 |
...
| ADR-171 | 2026-08-15 | Use a TypeScript Fastify REST API on Sydney Cloud Run, configured to scale to zero with maximum-instance/cost controls. | Superseded by ADR-272 |
| ADR-172 | 2026-08-15 | Route all MediFind business operations through the Cloud Run API; the mobile app has no direct Firestore/Storage operation access. | Superseded by ADR-272 |
...
| ADR-183 | 2026-08-16 | Target WCAG 2.2 AA for the mobile app and minimal public-support site, subject to documented exception only. | Superseded by ADR-272 |
...
| ADR-185 | 2026-08-16 | Require physical iOS VoiceOver and Android TalkBack validation for every critical buyer, pharmacy and admin beta journey. | Superseded by ADR-272 |
...
| ADR-195 | 2026-08-16 | Use one TypeScript monorepo with mobile, API and narrowly scoped shared contract/config packages. | Superseded by ADR-272 |
...
| ADR-197 | 2026-08-16 | Keep mobile/API independently deployable; shared packages contain contracts/config only and never secrets or direct data access. | Superseded by ADR-272 |
...
| ADR-206 | 2026-08-16 | Use generic direct FCM/APNs push as the primary status-update signal; notification open re-fetches authorised API state. | Superseded by ADR-272 |
...
| ADR-208 | 2026-08-16 | Do not use WebSockets, direct Firestore listeners or realtime-database subscriptions in MVP. | Superseded by ADR-272 |
...
| ADR-214 | 2026-08-16 | Use verified public branch location only for native-map directions; never disclose or retain buyer nearby-search location. | Superseded by ADR-272 |
...
| ADR-224 | 2026-08-16 | Begin implementation only with synthetic-data monorepo/tooling/CI/mobile-shell foundation; defer cloud/auth/production configuration. | Superseded by ADR-272 |
...
| ADR-231 | 2026-08-16 | Execute each prescription scan as a non-public Cloud Run Job using only an opaque scan-job ID. | Superseded by ADR-272 |
...
| ADR-240 | 2026-08-16 | Use regional API Gateway as the sole public business-API endpoint, validating Firebase JWTs and invoking IAM-private Cloud Run through a dedicated service account. | Superseded by ADR-272 |
| ADR-241 | 2026-08-16 | Defer external load balancing and Cloud Armor until legal/assessment, attack, public-scale, custom-domain or funded-budget triggers require them; prevent path bypass on upgrade. | Superseded by ADR-272 |
| ADR-242 | 2026-08-16 | Use EAS Free for low-volume signed beta builds without depending on EAS Update; paid EAS or founder-controlled builds scale the same native project. | Superseded by ADR-272 |
...
| ADR-244 | 2026-08-16 | Use ClamAV in bounded private Cloud Run Jobs with a controlled signature mirror; public malware-analysis services remain prohibited. | Superseded by ADR-272 |
| ADR-245 | 2026-08-16 | Use generic direct FCM/APNs and authenticated in-app status for buyer/pharmacy workflow notifications; no transactional-email notification provider is required in MVP. | Superseded by ADR-272 |
...
| ADR-248 | 2026-08-16 | Send identity/security email through Firebase Authentication templates using a verified founder-controlled MediFind sender/action-link domain; defer a separate SMTP/API provider until measured need and approval. | Superseded by ADR-272 |
...
| ADR-250 | 2026-08-16 | Build MVP medicine search from a server-generated minimum public Firestore projection behind a backend search adapter; add a private managed index only under measured quality/latency/volume triggers. | Superseded by ADR-272 |
...
| ADR-252 | 2026-08-16 | Every 15 minutes, use a private idempotent Sydney Cloud Scheduler trigger to a dedicated scale-to-zero Cloud Run maintenance service for bounded expiry, staleness, deletion, scan recovery and aggregate reconciliation; reads also enforce effective time state. | Superseded by ADR-272 |
| ADR-253 | 2026-08-16 | Use OpenTofu plus reviewed Firebase declarative configuration, private versioned Sydney GCS remote state and GitHub OIDC/Google Workload Identity Federation with no long-lived deployment keys. | Superseded by ADR-272 |
| ADR-254 | 2026-08-16 | Keep direct bounded Cloud Run Job scan execution for the pilot and add regional authenticated Cloud Tasks dispatch only when measured recovery/backlog/throughput requires it; tasks carry only opaque IDs. | Superseded by ADR-272 |
| ADR-255 | 2026-08-16 | Use a second Sydney Cloud Scheduler job every six hours for a least-privilege ClamAV signature-updater Job; scanners read but cannot alter the mirror, and definitions older than 24 hours fail closed. | Superseded by ADR-272 |
...
| ADR-266 | 2026-08-16 | Supersede the mobile-first product-surface and store-distribution direction. The current MVP client is a responsive TypeScript web application/PWA for desktop browsers, Android browsers and iPhone Safari, distributed through an invite-only HTTPS environment and optional Home Screen installation. **This row is itself superseded by ADR-272: the referenced native package is archived for traceability only, not retained as a future native-shell option; App Store, TestFlight, Google Play and EAS distribution are not future MediFind work.** | Superseded by ADR-272 |
| ADR-267 | 2026-08-16 | Approve the next Claude implementation task as the local synthetic web/PWA buyer-search experience in `apps/web`, reusing the historical Task 2 behaviour without authorising authentication, network/API calls, cloud resources, hosting, native packaging, store distribution or production capability. | Accepted |
| ADR-268 | 2026-08-16 | Accept the public-repository GitHub Free route for synthetic and future cloud-connected work after the founder declined a GitHub Pro upgrade. `main` must remain PR-only with required Quality checks, linear history, resolved conversations and no force/deletion pushes; no real data, production secret or sensitive workflow is authorised by repository visibility. | Accepted |
| ADR-269 | 2026-08-16 | Approve Cloudflare Pages Free only as a temporary HTTPS host for the synthetic web/PWA preview and iPhone Safari verification. The preview is static, public-synthetic-only, noindex by default and contains no API, database, authentication, cookies, analytics, Functions or production capability. | Accepted |
...
| ADR-275 | 2026-08-17 | Define the next synthetic D1 slice as six server-only search tables (`medicine_concepts`, `pharmacy_organisations`, `pharmacy_branches`, `medicine_listings`, `public_search_projection`, `public_search_terms`) with deterministic fixtures, field-level privacy classifications, Worker-owned authorization, logical JSONL export/checksums and additive migrations. Acceptance authorizes only a read-only synthetic `GET /v1/search` and `GET /v1/listings/{id}` implementation brief; it does not authorize a Cloudflare binding, account/database provisioning, real data, authentication provider, protected records, mutation, paid usage or production release. | Accepted |
| ADR-276 | 2026-08-17 | Record the founder's approval in this task to provision one isolated synthetic Cloudflare Worker and one synthetic D1 database for the reviewed Task 4 read-only search slice. The environment may contain only the invented export, must not add Pages hosting, accounts, authentication, secrets, R2, KV, mutations, protected/real data or production capability, and must remain within the documented free-first limits. Rollback is Worker deletion plus D1 deletion after export verification; exact resource and smoke evidence must be recorded in the infrastructure blueprint. | Accepted |
