...
- Provide touch targets and spacing suitable for touch and pointer use; support keyboard/focus interaction and avoid time-limited interaction where not essential. If a time limit is required for security/expiry, explain it and preserve safe recovery/renewal paths.
- Avoid gesture-only control paths. Ensure responsive layouts remain usable at narrow iPhone widths and wide desktop widths.
...
- Before each beta release, manually test every changed critical buyer, pharmacy and admin journey with iPhone Safari VoiceOver, Android browser TalkBack and a desktop keyboard/screen reader combination. Record browser/device, language, journey, issue and result.
